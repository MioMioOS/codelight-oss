...
final class SocketClient {
    ...
    func authenticate() async throws {
        ...
        #endif
        let (data, response) = try await URLSession.shared.codeLightData(for: urlRequest)
        #if DEBUG
    ...
    func connect() {
        ...
        manager = SocketManager(socketURL: url, config: [
            ...
            // X-Client-Version on the socket.io handshake (initial polling /
            // websocket upgrade HTTP request). Server doesn't gate sockets in
            // Phase 1/2, but this keeps the contract consistent across all
            // transports so future socket-side observation just works.
            .extraHeaders([ClientVersion.headerName: ClientVersion.headerValue]),
            .reconnects(true),
            .reconnectWait(1),
            .reconnectWaitMax(5),
    ...
    func redeemPairingCode(_ code: String) async throws -> LinkedDevice {
        ...
        guard let macId = result["macDeviceId"] as? String,
            ...
            // Same `message`-first preference as postJSON's HTTP error path.
            // postJSON already threw before us when status was non-2xx, so
            // hitting this guard means status was 200 but the body somehow
            // lacked macDeviceId/name — surface whatever the server said,
            // localized when available.
            let serverMessage = result["message"] as? String
            let errorKey = result["error"] as? String
            let msg = (serverMessage?.isEmpty == false ? serverMessage : nil)
                ?? errorKey
                ?? "Pairing failed"
    ...
    func unlinkDevice(_ deviceId: String) async throws {
        ...
        if let token { request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        let (_, response) = try await URLSession.shared.codeLightData(for: request)
    ...
    func fetchPresets(macDeviceId: String) async throws -> [LaunchPresetDTO] {
        ...
        var request = URLRequest(url: url)
        if let token { request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization") }
        let (data, response) = try await URLSession.shared.codeLightData(for: request)
    ...
    private func postJSON(path: String, body: [String: Any]) async throws -> [String: Any] {
        ...
        if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
            ...
            // Server returns two error fields by convention:
            //   `message` — human-readable (often localized Chinese), meant
            //               for direct display to the end user.
            //   `error`   — machine-readable key (e.g. `paired_mac_too_old`,
            //               `invalid_code`, `client_too_old`), meant for
            //               clients that want to switch on the case.
            // Prior to build 20 we surfaced `error` first, which leaked raw
            // keys like "paired_mac_too_old" into the UI for paying users
            // who scanned an outdated Mac (see server 594bcb1 pairing gate).
            // Prefer `message` whenever the server sent one; fall back to
            // the key only if the endpoint hasn't been upgraded to the new
            // envelope yet.
            let serverMessage = body["message"] as? String
            let errorKey = body["error"] as? String
            let msg = (serverMessage?.isEmpty == false ? serverMessage : nil)
                ?? errorKey
    ...
    func deleteAllPushTokens() async {
        ...
        _ = try? await URLSession.shared.codeLightData(for: request)
    }
