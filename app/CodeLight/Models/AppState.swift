...
final class AppState: ObservableObject {
    ...
    // MARK: - Client Version Gate
    /// Set true when the server returns HTTP 426 + `client_too_old`. RootView
    /// observes this and surfaces an upgrade prompt. The flag stays set after
    /// dismissal so subsequent rejections can re-display the alert.
    @Published var clientTooOld: Bool = false
    /// Optional informational message from the server's 426 response.
    @Published var clientTooOldMessage: String = ""
    ...
    private init() {
        ...
        observeStorePurchaseFlips()
        observeClientTooOld()
    }

    /// Listen for `.clientTooOld` notifications posted by `AppVersionGate`
    /// whenever a server response is HTTP 426 + `error: "client_too_old"`.
    /// Flips the `clientTooOld` flag so RootView's alert modifier fires.
    /// Runs on MainActor since AppState is @MainActor.
    private func observeClientTooOld() {
        NotificationCenter.default.addObserver(
            forName: .clientTooOld,
            object: nil,
            queue: .main
        ) { [weak self] note in
            Task { @MainActor in
                ...
                if let msg = note.userInfo?["message"] as? String, !msg.isEmpty {
                    self.clientTooOldMessage = msg
                }
                self.clientTooOld = true
            }
        }
    }

    /// OSS edition: no-op.
    private func observeStorePurchaseFlips() {}
    ...
    func connectToServer(url: String) async {
        ...
        do {
            ...
            // OSS edition: StoreKit retry call removed (no IAP).
    ...
    func pairWithCode(_ code: String, onServer serverUrl: String) async throws -> LinkedMac {
        ...
        // Server's propagateMacSubscriptionToPhone runs synchronously inside
        // /v1/pairing/code/redeem and emits a `subscription-updated` socket
        // event with the inherited trial. But if pairing forced a server
        // switch (currentServerUrl != serverUrl path above), the new socket
        // may not be fully ready when the event fires — the event is dropped
        // and the iPhone UI stays in its pre-pair "expired" state even though
        // the server DB already shows it as active.
        //
        // HTTP refetch as a belt-and-suspenders safety net so the UI always
        // lands on the correct state regardless of socket-event delivery
        // timing. This was reproduced specifically with QR-pair: scanning a
        // Mac QR sometimes triggered server URL switching whose new socket
        // wasn't ready when the propagation event fired.
