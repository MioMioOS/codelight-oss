...
struct SettingsView: View {
    ...
    // Observe StoreManager so isPurchased flips trigger SwiftUI re-render.
    // Direct singleton reads (storeManager.isPurchased) bypass observation
    // and leave the upgrade button stale after a successful purchase if the
    // server-verify push hasn't yet flipped appState.subscriptionStatus.
    ...
    @State private var restoreStatus: ActionStatus = .idle
    ...
    enum ActionStatus: Equatable {
        ...
        case running
        case success(String)
        case info(String)       // Neutral outcome (e.g., "no purchases to restore" — completed but found nothing).
        case failure(String)
    }
    ...
    var body: some View {
        List {
            ...
            Section {
                ...
                // Restore — ALWAYS visible (Apple Guideline 3.1.1).
                // Placement: BEFORE the Upgrade button so a reviewer's
                // top-down scan finds it first. Brand tint matches Upgrade's
                // prominence so Apple's "distinct Restore button" requirement
                // is met. Stays visible even when permanently paid so users
                // who reinstalled on a new device can re-trigger before
                // their local entitlement is restored.
                Button {
                    Task { await handleRestore() }
                } label: {
                    HStack {
                        Label(String(localized: "sub_restore_purchase"),
                              systemImage: "arrow.clockwise.circle")
                            .foregroundStyle(Theme.brand)
                        Spacer()
                        statusView(restoreStatus)
                    }
                }
                .disabled(restoreStatus == .running)

                // Hide upgrade only when permanently paid (no remaining-days expiry).
                // Trial OR redeem-code-active (active + daysLeft) both keep the upgrade
                // path visible — user/reviewer can always reach the IAP.
                let isPermanentlyPaid = storeManager.isPurchased
                    || (appState.subscriptionStatus == "active" && appState.trialDaysLeft == nil)
                if !isPermanentlyPaid {
                    Button {
                ...
                // Footer copy adapts to subscription state. Buyers see thanks +
                // restore-on-other-devices hint; trial/none see the buy CTA.
    ...
    // MARK: - Subscription Footer

    /// Footer text adapts to subscription state:
    ///   • Permanently paid → thanks + cross-device restore hint
    ///   • Trial / not paid → buy CTA explaining one-time purchase
    /// NSLocalizedString is used (not String(localized:)) because the key is
    /// chosen at runtime, which the compile-time-checked initializer rejects.
    private var subscriptionFooterText: String {
        let isPaid = storeManager.isPurchased
            || (appState.subscriptionStatus == "active" && appState.trialDaysLeft == nil)
        return isPaid
            ? NSLocalizedString("subscription_footer_paid", comment: "")
            : NSLocalizedString("subscription_footer", comment: "")
    }
    ...
    private var subscriptionBadge: some View {
        ...
        // "已激活" reserved for permanent paid (no expiry). Anything with remaining
        // days — whether server-side it's status=trial or status=active+daysLeft
        // (redeem code) — surfaces as "试用中 (剩余 N 天)" so user mental model is:
        //   has days = will end someday = "trial-like"
        //   no days  = permanent = "已激活"
        if storeManager.isPurchased || (status == "active" && appState.trialDaysLeft == nil) {
            Text(String(localized: "status_active"))
                .font(.caption)
                .fontWeight(.medium)
                .foregroundStyle(Theme.success)
        } else if let days = appState.trialDaysLeft, status == "active" || status == "trial" {
            Text(String(format: NSLocalizedString("trial_days_left_format", comment: ""), days))
                .font(.caption)
                .fontWeight(.medium)
                .foregroundStyle(Theme.warning)
        } else if status == "trial" {
            Text(String(localized: "status_trial"))
                .font(.caption)
                .fontWeight(.medium)
                .foregroundStyle(Theme.warning)
            ...
            // Pre-pair (no server URL) → status never resolves via socket, so
            // ProgressView would spin forever and Apple reviewers would see a
            // hung state next to "Subscription". Render nothing in that case;
            // the Upgrade button below conveys intent. Once paired, server push
            // flips status to trial/active/expired and the badge re-renders.
            if appState.currentServerUrl != nil {
                ProgressView()
                    .controlSize(.mini)
            }
        } else {
            // Defensive fallback: server may return future status values
            // (cancelled, revoked, pending, etc.) not yet known to this client.
            // Never let the badge render empty next to the crown icon.
            Text(String(localized: "status_trial"))
                .font(.caption)
                .fontWeight(.medium)
    ...
    private func statusView(_ status: ActionStatus) -> some View {
        Group {
            ...
            case .info(let msg):
                // Neutral outcome — not success, not failure. Used for
                // "operation completed, found nothing" results like
                // "no purchases to restore". A red xmark would imply
                // user fault; a green checkmark would falsely claim
                // success. Secondary-text icon + color reads as
                // informational.
                HStack(spacing: 4) {
    ...
    private func handleRestore() async {
        ...
        do {
            ...
            if storeManager.isPurchased {
                Haptics.success()
                restoreStatus = .success(String(localized: "sub_restore_success"))
            } else {
                // Empty-state is not failure — it's a neutral information
                // result. See ActionStatus.info comment + statusView .info
                // branch for why this distinction matters.
                Haptics.selection()
                restoreStatus = .info(String(localized: "sub_restore_no_purchase"))
            }
        } catch {
            Haptics.error()
            restoreStatus = .failure(error.localizedDescription)
        }
        // Auto-clear after a moment — matches reconnect / cleanup behavior.
        try? await Task.sleep(nanoseconds: 3_000_000_000)
        if case .success = restoreStatus { restoreStatus = .idle }
        if case .info = restoreStatus { restoreStatus = .idle }
        if case .failure = restoreStatus { restoreStatus = .idle }
    }
    ...
    private func runCleanup() async {
        ...
        do {
            let (data, _) = try await URLSession.shared.codeLightData(for: request)
