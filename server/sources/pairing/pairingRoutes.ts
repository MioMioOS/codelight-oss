...
// OSS edition: cross-device entitlement propagation is a no-op.
// The function is retained so existing call sites compile unchanged.
async function propagateMacSubscriptionToPhone(_macId: string, _phoneId: string): Promise<void> {
    // No-op.
}
...
export async function pairingRoutes(app: FastifyInstance) {
    ...
    app.post('/v1/pairing/respond', {
        ...
        if (initiatorDeviceId && initiatorDeviceId !== responderDeviceId) {
            ...
            // Retained call sites (no-op in OSS edition).
    ...
    app.post('/v1/pairing/code/redeem', {
        ...
        // Retained call site (no-op in OSS edition).
